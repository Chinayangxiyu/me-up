...
各CPU和内存的公共通信干线。
## MESI协议
将缓存的变量分为几个状态修改（Modify），独占（Exclusive）、共享（shard）、无效（Invalid）。
以两个CPU举例：一个CPU读取变量三独占，两个CPU读取是共享，一个CPU更新变量当前CPU是修改，其它CPU是失效状态。

CPU通过嗅探机制去监听总线上的变量，来检查自己缓存的变量是否过期来实现MESI协议。
## 嗅探机制
监听总线上的变量
