...
**&emsp;&emsp;  【补充】：HashMap的Entry元素，在产生哈希中途后，如果阈值超过8会转为红黑树，树节点的存储需要和节点的值**  
...
哈希冲突解决方法：开发寻址、再散列、链表、建立公共溢出区。  
（1）开放寻址：遇到哈希冲突，根据随机、顺序、左右跳跃式探测的方式找一个空的地址保存冲突值。  
（2）再散列：多次散列知道找到适用的地址。  
（3）链表：链表或者树保存冲突值，（如HashMap，但是冲突保存在树节点的时候会再进行一次hash计算，确定节点的值）  
（4）建立公共溢出区：溢出区存放冲突值。
...
&emsp;&emsp;    1.8使用红黑树+链表替换纯链表的优势：如果hashCode方法分布均匀，产生哈希冲突的概率极低，文档中给出的数据是(0.00000006)；当我们自己  
            ...
            提升效率。  
&emsp;&emsp;    加载因子0.75是空间和哈希冲突的一个折衷选择，加载因子大可以节省空间，但是哈希冲突为提高；加载因子太小会浪费空间。  
&emsp;&emsp;    链表转树阈值为8，阈值为8产生哈希冲突的概率是0.00000006。
